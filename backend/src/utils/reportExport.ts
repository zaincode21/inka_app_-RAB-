...
/** Designed CSV: farm header block, then blank line, then data table. */
export function toCsvWithFarmHeader(
  farm: FarmReportInfo,
  meta: CsvReportMeta,
  headers: string[],
  ...
  const preamble: Array<[string, string]> = [
    ...
    ['Phone', farm.ownerPhone?.trim() || ''],
    ['Location', farm.location],
    ['District', farm.district],
    ['Sector', farm.sector],
    ['Currency', farm.currency],
    ['Period From', meta.periodFrom],
    ['Period To', meta.periodTo],
    ['Generated At', generated.toISOString()],
  ];
  if (meta.generatedBy?.trim()) {
    preamble.push(['Generated By', meta.generatedBy.trim()]);
  }

  const lines = preamble.map(([label, value]) => `${csvEscape(label)},${csvEscape(value)}`);
  lines.push('');
